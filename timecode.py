...
def frames_to_timestamp(frame, fps=30):
    """Convert frame number to timestamp in SRT format (hh:mm:ss;ff)."""
...
def process_ocr_to_subtitle(input_file, output_file, fps=30):
    """Process OCR frames and generate subtitle in expected format."""
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write('\n'.join(subtitles))
    print(f"Subtitle saved to {output_file}")

# 使用方法
input_txt = "apple.mp4_1.txt"  # 输入的txt文件路径
output_srt = "/home/kevin/TinyLLaVA_Factory/timecode.txt"  # 输出的字幕文件路径
